                    ...
                    <div class="glass-card" style="padding: 1.75rem; border-radius: 16px; border: 1px solid var(--border-color); background: var(--bg-card); box-shadow: 0 4px 15px rgba(0,0,0,0.05); display: flex; flex-direction: column; transition: transform 0.2s, box-shadow 0.2s;" onmouseover="this.style.transform='translateY(-6px)'; this.style.boxShadow='0 12px 25px var(--gradient-glow)';" onmouseout="this.style.transform='translateY(0)'; this.style.boxShadow='0 4px 15px rgba(0,0,0,0.05)';">
                        <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 1.25rem;">
                            ...
                            <div style="text-align: right;">
                                <div style="font-size: 1.15rem; color: var(--text-main); font-weight: 800;">
                                    Rp {{ number_format($tournament->price_per_slot, 0, ',', '.') }}
                                </div>
                        ...
                        <div style="margin-bottom: 1.5rem; background: var(--bg-input); border-radius: 12px; padding: 1rem;">
                            <div style="display: flex; justify-content: space-between; font-size: 0.85rem; font-weight: 600; margin-bottom: 0.75rem;">
                                <span style="color: var(--text-muted);">Sisa Slot: <span style="color: var(--text-main);">{{ $availSlots }}</span></span>
                                <span style="color: var(--text-muted);">Total: <span style="color: var(--text-main);">{{ $tournament->max_entries }}</span></span>
                            </div>
                            <div style="height: 8px; background: rgba(0, 0, 0, 0.1); border-radius: 10px; overflow: hidden; box-shadow: inset 0 1px 2px rgba(0,0,0,0.1);">
                                <div style="height: 100%; background: var(--primary); width: {{ $filledPercent }}%; border-radius: 10px;"></div>
                            </div>
                        </div>
                        
                        <a href="{{ route('player.login') }}" class="btn btn-primary" style="width: 100%; text-align: center; padding: 0.85rem; border-radius: 10px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px;">
                            Detail & Daftar
                        </a>
                    </div>
                @endforeach
            ...
            <div class="grid grid-cols-1 md-grid-cols-2" style="margin-top: 3rem; gap: 2rem; display: grid;">
                ...
                <div>
                    ...
                    <div style="display: flex; flex-direction: column; gap: 1rem; padding-right: 0.5rem;">
                        @forelse ($tournament->upcomingMatches as $match)
                            @php
                ...
                <div>
                    ...
                    <div style="display: flex; flex-direction: column; gap: 1rem; padding-right: 0.5rem;">
                        @forelse ($tournament->completedMatches as $match)
                            @php
                                $homePart = $match->participants->where('side', 'home')->first();
                                $awayPart = $match->participants->where('side', 'away')->first();
                                $homeName = $homePart?->entry?->display_name ?? 'TBD';
                            ...
                            <div class="soft-well" style="padding: 1rem 1.25rem; border-left: 3px solid var(--border-color); border-radius: 12px; opacity: 0.8;">
                                ...
                                <div style="display: flex; align-items: center; font-weight: 700; font-size: 0.95rem;">
                                    <div style="flex: 1; min-width: 0; {{ $homePart && $homePart->is_winner ? 'color: var(--primary);' : '' }}">
                                        <div style="white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">{{ $homeName }}</div>
                                        @if($homePart?->club?->name)
                                            <div style="font-size: 0.7rem; color: var(--text-muted); font-weight: 500; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">{{ $homePart->club->name }}</div>
                                        @endif
                                    ...
                                    <div style="flex-shrink: 0; text-align: center; padding: 0 0.75rem; min-width: 70px;">
                                        <div style="font-size: 1.1rem; letter-spacing: 2px;">{{ $homeScore }} - {{ $awayScore }}</div>
                                        @if($match->decided_by_penalty)
                                            <div style="font-size: 0.65rem; letter-spacing: 1px; color: var(--text-muted);">({{ $match->penalty_score_home }}) - ({{ $match->penalty_score_away }})</div>
                                        @endif
                                    </div>
                                    <div style="flex: 1; min-width: 0; text-align: right; {{ $awayPart && $awayPart->is_winner ? 'color: var(--primary);' : '' }}">
                                        <div style="white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">{{ $awayName }}</div>
                                        @if($awayPart?->club?->name)
                                            <div style="font-size: 0.7rem; color: var(--text-muted); font-weight: 500; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">{{ $awayPart->club->name }}</div>
                                        @endif
            ...
            <div class="glass-card" style="padding: 2rem; border-radius: 16px; overflow-x: auto;">
                ...
                <div style="display: flex; justify-content: space-between; align-items: flex-start; flex-wrap: wrap; gap: 1rem; margin-bottom: 1.5rem; padding-bottom: 1.5rem; border-bottom: 1px solid var(--border-color);">
                    <div>
                        ...
                        <p style="color: var(--text-muted); font-size: 0.95rem;">{{ $latestTournament->game_title }}</p>
                    </div>
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem; margin-bottom: 2rem;">
                    <div style="background: var(--bg-input); padding: 1rem; border-radius: 10px;">
                        ...
                        <div style="font-size: 0.95rem; font-weight: 700; color: var(--text-main);">{{ $latestTournament->registration_end->format('d M Y H:i') }} WIB</div>
                    </div>
                    <div style="background: var(--bg-input); padding: 1rem; border-radius: 10px;">
                        ...
                        <div style="font-size: 0.95rem; font-weight: 700; color: var(--primary);">{{ $latestTournament->tournament_start->format('d M Y H:i') }} WIB</div>
                    </div>
                    <div style="background: var(--bg-input); padding: 1rem; border-radius: 10px;">
                        ...
                        <div style="font-size: 0.95rem; font-weight: 700; color: var(--text-main);">Rp {{ number_format($latestTournament->price_per_slot, 0, ',', '.') }}</div>
                    </div>
                    <div style="background: var(--bg-input); padding: 1rem; border-radius: 10px;">
                        ...
                        <div style="font-size: 0.95rem; font-weight: 700; color: var(--text-main);">{{ $latestTournament->max_entries }}</div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md-grid-cols-2" style="gap: 2rem;">
                    ...
                    <div>
                        ...
                        <div style="display: flex; flex-direction: column; gap: 0.75rem;">
                            @forelse ($latestTournament->upcomingMatches as $match)
                                @php
                                    $homePart = $match->participants->where('side', 'home')->first();
                                    $awayPart = $match->participants->where('side', 'away')->first();
                                    $homeName = $homePart?->entry?->display_name ?? 'TBD';
                                ...
                                <div class="soft-well" style="padding: 0.85rem 1rem; border-left: 3px solid {{ $match->status === 'ongoing' ? 'var(--primary)' : 'var(--border-color)' }}; border-radius: 10px;">
                                    ...
                                    <div style="display: flex; align-items: center; font-weight: 700; font-size: 0.88rem;">
                                        ...
                                        <div style="flex-shrink: 0; text-align: center; padding: 0 0.6rem; min-width: 60px;">
                                            @if($match->status === 'ongoing' || $match->status === 'completed' || $match->status === 'walkover')
                                                <div style="font-size: 1rem; letter-spacing: 2px; font-weight: 800; color: var(--primary);">{{ $homePart?->goals_scored ?? 0 }} - {{ $awayPart?->goals_scored ?? 0 }}</div>
                                            @else
                    ...
                    <div>
                        ...
                        <div style="display: flex; flex-direction: column; gap: 0.75rem;">
                            @forelse ($latestTournament->completedMatches->take(10) as $match)
                                @php
                                    $homePart = $match->participants->where('side', 'home')->first();
                                    $awayPart = $match->participants->where('side', 'away')->first();
                                    $homeName = $homePart?->entry?->display_name ?? 'TBD';
                                ...
                                <div class="soft-well" style="padding: 0.85rem 1rem; border-left: 3px solid var(--border-color); border-radius: 10px; opacity: 0.8;">
                                    ...
                                    <div style="display: flex; align-items: center; font-weight: 700; font-size: 0.88rem;">
                                        ...
                                        <div style="flex-shrink: 0; text-align: center; padding: 0 0.6rem; min-width: 60px;">
                                            <div style="font-size: 1rem; letter-spacing: 2px;">{{ $homeScore }} - {{ $awayScore }}</div>
                                        </div>
                                        <div style="flex: 1; min-width: 0; text-align: right; {{ $awayPart && $awayPart->is_winner ? 'color: var(--primary);' : '' }}">
                                            <div style="white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">{{ $awayName }}</div>
                                        </div>
                    ...
                    @forelse($topPlayers as $index => $player)
                        <div style="display: flex; justify-content: space-between; align-items: center; padding: 1.25rem 1.5rem; border-bottom: 1px solid var(--border-color); transition: background 0.2s;" onmouseover="this.style.background='var(--gradient-glow)';" onmouseout="this.style.background='transparent';">
                            <div style="display: flex; align-items: center; gap: 1.25rem;">
                                <div style="font-size: 1.1rem; font-weight: 800; color: {{ $index === 0 ? '#FBBF24' : ($index === 1 ? '#9CA3AF' : ($index === 2 ? '#D97706' : 'var(--text-muted)')) }}; width: 32px; height: 32px; display: flex; align-items: center; justify-content: center; background: {{ $index === 0 ? 'rgba(251,191,36,0.1)' : ($index === 1 ? 'rgba(156,163,175,0.1)' : ($index === 2 ? 'rgba(217,119,6,0.1)' : 'transparent')) }}; border-radius: 8px;">
                                    {{ $index + 1 }}
                                </div>
                                <div>
                                    <div style="font-weight: 800; font-size: 1.05rem; color: var(--text-main); margin-bottom: 0.15rem;">{{ $player->name }}</div>
