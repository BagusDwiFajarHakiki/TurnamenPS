    ...
    <div class="turnamen-recap-admin" style="background-color: #0d1117; color: #c9d1d9; padding: 2rem; border-radius: 12px; font-family: 'Inter', sans-serif;">
        ...
        <div class="recap-header">
            ...
            <div style="display: flex; gap: 1rem;">
                ...
                <span style="background-color: rgba(57, 211, 83, 0.1); color: #39d353; padding: 0.5rem 1rem; border-radius: 8px; font-weight: 700; font-size: 0.85rem;">
                    {{ $this->record->entries()->count() }} / {{ $this->record->max_entries }} Slot
                </span>
            </div>
        </div>
        ...
        <div class="recap-stats-grid">
            <div class="glass-card" style="padding: 1.5rem; border-radius: 12px; background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.05);">
                ...
                <div style="color: var(--text-main); font-size: 1.1rem; font-weight: 700;">{{ \Carbon\Carbon::parse($this->record->registration_end)->format('d M Y H:i') }} WIB</div>
            </div>
            ...
            <div class="glass-card" style="padding: 1.5rem; border-radius: 12px; background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.05);">
                ...
                <div style="color: var(--primary, #39d353); font-size: 1.1rem; font-weight: 700;">{{ \Carbon\Carbon::parse($this->record->tournament_start)->format('d M Y H:i') }} WIB</div>
            </div>
            ...
            <div class="glass-card" style="padding: 1.5rem; border-radius: 12px; background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.05);">
                ...
                <div style="color: var(--text-main); font-size: 1.1rem; font-weight: 700;">Rp {{ number_format($this->record->price_per_slot, 0, ',', '.') }}</div>
            </div>
            ...
            <div class="glass-card" style="padding: 1.5rem; border-radius: 12px; background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.05);">
                ...
                <div style="color: var(--text-main); font-size: 1.1rem; font-weight: 700;">{{ $this->leaderboard->count() }} Orang</div>
            </div>
        </div>
        ...
        <div class="recap-content-grid">
                ...
                <div style="display: flex; flex-direction: column; gap: 1.5rem;">
                    @foreach($leaderboard->take(3) as $index => $row)
                        <div style="display: flex; align-items: center; gap: 1rem;">
                            ...
                            <div>
                                <div style="font-size: 0.8rem; color: var(--text-muted); text-transform: uppercase; font-weight: 700;">Juara {{ $index + 1 }}</div>
                                <div style="font-size: 1.2rem; font-weight: 700;">{{ $row->player->name }}</div>
                            </div>
                        </div>
                    @endforeach
            ...
            <div class="glass-card" style="padding: 1.5rem; border-radius: 12px; background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.05); position: relative; overflow: hidden;">
                ...
                <div style="display: flex; flex-direction: column; gap: 1rem;">
                    @foreach($topScorers->take(5) as $index => $row)
                        <div style="display: flex; justify-content: space-between; align-items: center; padding-bottom: 0.75rem; border-bottom: 1px solid rgba(255,255,255,0.1);">
                            <div style="font-weight: 600; font-size: 1rem;">{{ $index + 1 }}. {{ $row->player->name }}</div>
                            <div style="font-weight: 700;">{{ $row->total_goals_scored }} Gol</div>
                        </div>
                    @endforeach
