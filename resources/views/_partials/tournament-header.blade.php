...
<div style="display: flex; justify-content: space-between; align-items: flex-start; flex-wrap: wrap; gap: 1rem; margin-bottom: 1.5rem; padding-bottom: 1.5rem; border-bottom: 1px solid var(--border-color);">
    <div>
        ...
        <p style="color: var(--text-muted); font-size: 0.95rem;">{{ $tournament->game_title }}</p>
    </div>
...
<div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem; margin-bottom: 2rem;">
    <div style="background: var(--bg-input); padding: 1rem; border-radius: 10px;">
        ...
        <div style="font-size: 0.95rem; font-weight: 700; color: var(--text-main);">{{ $tournament->registration_end->format('d M Y H:i') }} WIB</div>
    </div>
    <div style="background: var(--bg-input); padding: 1rem; border-radius: 10px;">
        ...
        <div style="font-size: 0.95rem; font-weight: 700; color: var(--primary);">{{ $tournament->tournament_start->format('d M Y H:i') }} WIB</div>
    </div>
    <div style="background: var(--bg-input); padding: 1rem; border-radius: 10px;">
        ...
        <div style="font-size: 0.95rem; font-weight: 700; color: var(--text-main);">Rp {{ number_format($tournament->price_per_slot, 0, ',', '.') }}</div>
    </div>
    <div style="background: var(--bg-input); padding: 1rem; border-radius: 10px;">
