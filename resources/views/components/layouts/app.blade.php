...
<body style="display: flex; flex-direction: column; min-height: 100vh; margin: 0;">
    ...
    <nav class="{{ request()->is('/') ? 'nav-at-top' : '' }}" id="mainNav" data-is-home="{{ request()->is('/') ? 'true' : 'false' }}" style="flex-shrink: 0; width: 100%; padding: 0;">
        <div style="max-width: 1200px; margin: 0 auto; width: 100%; display: flex; justify-content: space-between; align-items: center; padding: 1rem clamp(0.75rem, 3vw, 1.5rem);">
            ...
            <div class="nav-links">
                @auth('player')
                    <a href="{{ route('player.dashboard') }}" wire:navigate class="nav-link nav-auth-link {{ request()->is('dashboard') ? 'active' : '' }}">{{ __('Dasbor') }}</a>
                @else
                    <a href="{{ route('player.login') }}" wire:navigate class="nav-link nav-auth-link {{ request()->routeIs('player.login') ? 'active' : '' }}">{{ __('Login') }}</a>
                @endauth
